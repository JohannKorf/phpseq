...
final class ComponentsCommand extends Command
{
    ...
    {
        $srcRoots = array_map([$this, 'expandPathLoose'], (array) $input->getOption('src'));
        $outPath = (string)$input->getOption('out');
        $out = (string) $input->getOption('out');
        if ($out !== '') {
            $out = $this->expandPathLoose($out);
            $outDir = dirname($out);
        ...
        $drillDir = $input->getOption('drilldown-dir') ?: null;
        if ($drillDir) {
            $drillDir = $this->expandPathLoose($drillDir);
            if (!is_dir($drillDir) && !mkdir($drillDir, 0777, true) && !is_dir($drillDir)) {
                throw new \RuntimeException("Failed to create drilldown dir: {$drillDir}");
            }
        }
        foreach ($srcRoots as $srcRoot) {
            if (!is_dir($srcRoot)) {
                $output->writeln("<error>Source root '$srcRoot' does not exist or is not a directory.</error>");
    ...
    private function expandPathLoose(string $path): string
    {
        $p = trim($path);
        ...
        $p = preg_replace_callback('/\\$(\\w+)|\\$\\{([^}]+)\\}/', function ($m) {
            $var = $m[1] ?? $m[2];
            $val = getenv($var);
            return $val === false ? $m[0] : $val;
        }, $p);
        return preg_replace('#^file://#', '', $p);
     }
}
